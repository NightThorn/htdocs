					...
					<?php if (!get_data($row, 'sub_menu')) { ?>
						<div class="menu-item <?php _e(segment(1) == get_data($row, 'id') ? 'active' : '') ?>">
							<a href="<?php _e(get_url(get_data($row, 'id'))) ?>">
								<span class="menu-icon"><img height="30" style="margin-bottom: 10px; margin-right: 2px;" src="<?php _e(get_data($row, 'img')) ?>"><i class="<?php _e(get_data($row, 'icon')) ?>" style="<?php _e((get_option('appearance_icon_color', 0) &&  get_data($row, 'color')) ? "color: " . get_data($row, 'color') : "") ?>"></i></span>
